...
class Transformer:
    ...
    def compute_metrics(self,df):
        df['Daily return']=(df['Close']-df['Close'].shift(1))/df['Close'].shift(1)
        df['average7'] = df['Close'].rolling(window=7).mean() #moving average
        df['average21']=df['Close'].rolling(window=21).mean()
        return df
